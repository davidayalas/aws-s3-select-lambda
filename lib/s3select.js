    ...
    S3.selectObjectContent(params, (err, data) => {
      ...
      eventStream.on('end', () => {
        ...
        if(params.OutputSerialization.CSV){
          resolve(`[[${Buffer.concat(records).toString('utf8').replace(/\r/g,"").split(params.OutputSerialization.CSV.RecordDelimiter).join("],[")}]]`);
...
exports.query = async (setup) => {
  ...
  if(setup.OutputSerialization && setup.OutputSerialization.JSON){
    default_params.OutputSerialization = {};
  }

  if(setup.InputSerialization && setup.InputSerialization.JSON){
    default_params.InputSerialization = {};
  }
